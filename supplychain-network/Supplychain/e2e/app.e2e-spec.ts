...
describe('Starting tests for Supplychain', function() {
  ...
  it('network-name should be supplychain-network@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('supplychain-network@0.0.1.bna');
    });
  });
  ...
  it('navbar-brand should be Supplychain',() => {
    ...
    it('TempratureReading component should be loadable',() => {
      page.navigateTo('/TempratureReading');
      browser.findElement(by.id('transactionName'))
      .then((transactionName) => {
        return transactionName.getText();
      })
      .then((txt) => {
        expect(txt).toBe('TempratureReading');
      });
    });
